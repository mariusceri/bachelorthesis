...
function tau_lw(i_1, i_2, params) # i_1 > i_2
...
function lw_up(flux_down , t_s, temp, params)
    ...
    flux_up_lw[1] = params.eps*planck(t_s, params) + (1-params.eps)*flux_down[1]
...
function temp_ev(t_f, params, flux_tot_sw)
    ...
    for t in 1:dt:t_f
        ...
        # flux_up_lw + flux_up_sw = flux_down_lw + flux_down_sw   
        # (1-emissiv)*flux_down_lw + emiss * sigma Ts^4 + flux_up_sw = flux_down_lw + flux_down_sw
        #  emiss * sigma Ts^4 = emissiv*flux_down_lw + flux_down_sw*(1-albedo)
        t_s = sqrt(sqrt((flux_down_lw[1]-flux_tot_sw[1]/params.eps)/params.stephan))
#        @info "surface lw flux" flux_down_lw[1]
#        @info "surface temp" t_s

        flux_up_lw = lw_up(flux_down_lw, t_s, temp, params)

        flux_tot_lw = flux_up_lw - flux_down_lw

#        @info "flux_lw" flux_down_lw flux_up_lw
#        error()
...
function main()
    params = (n = 100, solarc = 1340/4, alb = 0.32, ps=101325, coefvis=0.9, coefir = 0.08, eps = 0.9, cpp = 1000.0005, g = 9.81, stephan = 5.67e-8)
#    params = (n = 20, solarc = 1340/4, alb = 0.0, ps=101325, coefvis=1.0, coefir = 1.0, eps = 1.0, cpp = 1000.0005, g = 9.81, stephan = 5.67e-8)
    c_sw = -.5*log(params.coefvis)/params.ps # tau0
    c_lw = -log(params.coefir)/sqrt((params.ps)^2/(2*params.g)) # == zcoef
    @info "absorption coefficients" c_sw c_lw
