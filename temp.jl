...
c_lw = -log(params.coefir)/sqrt(params.ps*params.ps/(2*params.g))
...
function new_ts(flux)
    return sqrt(sqrt(flux[1]/params.stephan))
end
...
function lw_up(flux_down , t_s, temp, flux_up_lw=zeros(n+1))
    ...
    for i in 2:n+1
        for k in 1:i-1
            flux_up_lw[i] += (tau(k+1,i) - tau(k,i))*planck(temp[k]) + tau(1,i)*flux_up_lw[1]
        end
    end
    return flux_up_lw
end
